...
function smartPaste() {
    ...
    let selectedItem = selectedItems.slice()[0];
    const pastedItems = [];
    const notification = figma.notify('Processing...', { timeout: 32 });
    for (let id of ids) {
        const clipboardItem = figma.getNodeById(id);
        if (!clipboardItem) {
...
function _pasteItem(item, parent) {
    ...
    const point = getPositionRelativeToNode(item, _parent);
    _parent.appendChild(newItem);
    newItem.x = point.x;
    newItem.y = point.y;
    return newItem;
}
function getPositionRelativeToNode(source, target) {
    let _source = source;
    let _target = target;
    ...
    let point = { x: _source.x, y: _source.y };
